...
#   são somadas INDEPENDENTE da parede hospedeira
# ===============================
grupos      = {}
ignorados   = []
sem_dimensao = []
...
for el in elements:
    ...
    # chave SEM marca_hospedeiro → agrupa entre paredes diferentes
    chave = (particao, marca_tipo_tela, l_key, c_key)
    ...
    if chave not in grupos:
        grupos[chave] = {
            "folhas":           [],
            "elementos":        [],
            "paredes":          set(),   # paredes que contribuíram
            "particao":         particao,
    ...
    grupos[chave]["paredes"].add(marca_hospedeiro)


# ===============================
# ORDENAÇÃO
# Partição → menor folha → tipo tela → dimensões
# ===============================
...
for chave, dados in grupos_ordenados:
    ...
    paredes_lista   = sorted(dados["paredes"], key=ordem_marca)
    ...
    # Lista as paredes que contribuíram + quantidade total somada
    paredes_str = ", ".join(paredes_lista) if paredes_lista else "-"
