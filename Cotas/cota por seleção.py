...
__version__ = "2.0"
__doc__ = ("cota - v2.0: unidade principal passa a ser a PAREDE. Toda "
           "parede e' processada individualmente e SEMPRE recebe cota "
           "geral (+ subcotas quando houver detalhe). Correntes/alinhamentos "
           "deixam de decidir se uma parede e' cotada - agora servem so "
           "para layout de alinhamento_total/geral.")
...
def coletar_elementos_relacionados(view, paredes_selecionadas):
    ...
    try:
        ...
        logger.error("Falha ao coletar portas/janelas da vista: {}".format(e))
        forms.alert("Falha ao coletar portas/janelas da vista:\n{}".format(e), exitscript=True)
        return [], []
    ...
    for fi in familias:
        try:
            ...
            if bic != int(BuiltInCategory.OST_Doors) and bic != int(BuiltInCategory.OST_Windows):
                continue
            host = fi.Host
            if host is None or host.Id.IntegerValue not in wall_ids:
                continue
            portas_janelas.append(fi)
        except Exception as e:
            logger.debug("Falha ao avaliar FamilyInstance {}: {}".format(fi.Id.IntegerValue, e))

    elementos.extend(portas_janelas)
    ...
    output.print_md(
        "**{} parede(s) selecionada(s)** | **{} porta(s)/janela(s) hospedada(s)**".format(
            len(paredes_selecionadas), len(portas_janelas)
        )
    )
...
def gerar_tarefas_de_cota(correntes, itens_todos, tolz):
    """[v2.0] Camadas por alinhamento - NOTE: 'parede_total' (cota geral de
    cada parede) e as 'vaos' de cada parede NAO sao mais geradas aqui.
    Desde a v2.0 isso e' responsabilidade exclusiva de
    ...
    O que esta funcao ainda gera, por corrente/alinhamento:
      ...
      'alinhamento_total'       - cota de conjunto da fileira (varias
                                   ...
                                   propria cota (PASSO 5 da nova
                                   estrategia) - nunca substitui a cota
    ...
    for c in correntes:
        ...
        # [v2.0] parede_total/vaos por parede NAO sao mais decididos aqui.
        # Toda parede e' processada de forma independente em
        # processar_paredes_individualmente (PASSO 1-4 da nova estrategia):
        # ela SEMPRE recebe sua cota geral, mesmo se estiver sozinha nesta
        # corrente (o que antes fazia a corrente inteira ser descartada).
        # A corrente aqui so serve para decidir alinhamento_total/geral
        # (PASSO 5: cotas de conjunto, depois de todas as paredes prontas).
...
def processar_paredes_individualmente(
    ...
    As "correntes"/alinhamentos (ETAPA 5, gerar_tarefas_de_cota) deixam de
    decidir se uma parede sera cotada - isso agora e' feito 100% aqui, por
    parede, de forma independente. Correntes continuam existindo so para
    organizar alinhamento_total/geral (PASSO 5 da nova estrategia).
    """
...
def main():
    ...
    # [v2.0] PASSO 1-4 da nova estrategia: cada parede e' cotada de forma
    # INDEPENDENTE primeiro - toda parede sempre ganha sua cota geral (+
    # subcotas se tiver detalhe), nao importa se esta isolada ou nao tem
    # nenhuma outra parede alinhada com ela.
